...
class Sudoku:
    ...
    def __init__(self, master):
        ...
        self.grid = {}
        # A small edit window which will be initialized and displayed on click
        self.e = None
        ...
        self.btn_gen = tk.Button(
            master, text="Generate", command=self.Generate, width=8
        )
        self.btn_gen.grid(row=1, column=1, padx=5, pady=5, sticky=tk.E)
        # Difficulty selector
        self.set_difficulty = tk.IntVar(master, 1)
        self.difficulty_selector = tk.OptionMenu(
            master, self.set_difficulty, 1, 2, 3, 4, 5
        )
        ...
        for x in range(1, 9):
            ...
            if x % 3 == 0:
                # Draw thicker black lines for separating 3x3 boxes
                width = 2
    ...
    def click(self, eventorigin):
        ...
        coords = [
            rect_x,
            rect_y,
            rect_x + self.cell_width,
            rect_y,
            rect_x + self.cell_width,
            rect_y + self.cell_height,
            rect_x,
            rect_y + self.cell_height,
        ]
        # Get cell info
        ...
        if editable:
            ...
            h_box = self.canvas.create_polygon(
                coords, outline=self.hbox_green, fill="", width=3
            )
            ...
            h_box = self.canvas.create_polygon(
                coords, outline=self.hbox_red, fill="", width=3
            )
            self.canvas.after(200, lambda: self.canvas.delete(h_box))

    def edit(self, cordx: int, cordy: int):
        # Create an entry inside a small canvas window
        # make sure it's actually initialized before deleting it
        if self.e is None:
            pass
        else:
            self.canvas.delete(self.e)
        # Create a mini edit window that just fits the cell
        self.e = self.canvas.create_window(
            cordx + 1,
            cordy + 1,
            window=self.t,
    ...
    def keyPressed(self, event):
        ...
        try:
            ...
            x, y = (self.t.winfo_x()) / self.cell_width, (
                self.t.winfo_y()
            ) / self.cell_height
        ...
        for i in range(9):
            for j in range(9):
                ...
                if val == 0:
                    ...
                    t = c.create_text(
                        text_x, text_y, text=val, font=("Times", 15, "bold")
                    )
    ...
    def Generate(self, level=1):
        ...
        cover = self.canvas.create_window(
            0,
            0,
            window=cover_label,
            width=self.canvas_width,
        ...
        # Reset count
        count = 0
        # Remove random numbers based on set difficulty level
...
# Here's an extreme puzzle, ref : https://www.sudokuwiki.org/Daily_Sudoku
ex2 = [
    [0, 5, 0, 0, 0, 0, 0, 0, 0],
    [3, 0, 8, 0, 7, 0, 2, 0, 0],
    [0, 0, 9, 3, 0, 6, 8, 0, 0],
